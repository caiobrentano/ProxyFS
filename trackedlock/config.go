...
func Up(confMap conf.ConfMap) (err error) {
	...
	// if the lock checke is disabled or there's no time limit then
...
func ExpandAndResume(confMap conf.ConfMap) (err error) {
	...
	logger.Infof("trackedlock lock check period/lock hold time limit changing from %d/%d sec to %d/%d sec",
		oldCheckPeriod/time.Second, oldTimeLimit/time.Second,
		globals.lockCheckPeriod/time.Second, globals.lockHoldTimeLimit/time.Second)
